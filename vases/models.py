...
from vases.arche_utils import get_results, get_ttl
from .config import (
    ARCHE_URI_CONST, COLLECTION_ABBR, THUMB_SERVICE, ACDHID_DOMAIN
)

ARCHE_NS = settings.ARCHE_NS
ID_PROP = settings.ARCHE_ID_PROP

def bin_by_type(bin_list, bin_type="Photos"):
    return [x for x in bin_list if bin_type in x]
...
class Object(models.Model):
    ...
    def get_binary_ids(self):
        g = get_ttl(self.get_basic_query_params())
        arche_ids = [
            f"{x[1]}" for x in g.subject_objects(
                predicate=rdflib.URIRef(ID_PROP)
            ) if f"{self.get_arche_url()}" in f"{x[1]}"
        ]
        return arche_ids
